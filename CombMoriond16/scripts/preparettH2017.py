...
parser.add_argument(
    '-i', help='path to cadi directory in cmshcg svn', default='.')
...
t_pre = os.path.join(
    args.i, 'cadi/HIG-17-003/preliminary_cards_hig17003_37fb/')
t_cards = [
    'ttH_1l_2tau.txt',
    'ttH_2lss_1tau.txt',
    'ttH_3l_1tau.txt'
...
l_pre = os.path.join(
    args.i, 'cadi/HIG-17-004/')
l_cards = [
    'ttH_2lss_ee_neg.card.txt',
    'ttH_2lss_ee_pos.card.txt',
    'ttH_2lss_em_bl_neg.card.txt',
    'ttH_2lss_em_bl_pos.card.txt',
    'ttH_2lss_em_bt_neg.card.txt',
    'ttH_2lss_em_bt_pos.card.txt',
    'ttH_2lss_mm_bl_neg.card.txt',
    'ttH_2lss_mm_bl_pos.card.txt',
    'ttH_2lss_mm_bt_neg.card.txt',
    'ttH_2lss_mm_bt_pos.card.txt',
...
b_pre = os.path.join(args.i, 'cadi/HIG-16-038/ttHbb_13TeV_HIG-16-038/')
b_cards = [
    'ttH_hbb_13TeV_dl_3j3t.txt',
    'ttH_hbb_13TeV_dl_ge4j3t_high.txt',
    'ttH_hbb_13TeV_dl_ge4j3t_low.txt',
    'ttH_hbb_13TeV_dl_ge4jge4t_high.txt',
    'ttH_hbb_13TeV_dl_ge4jge4t_low.txt',
    ...
    'ttH_hbb_13TeV_sl_jge6_t3_high.txt',
    'ttH_hbb_13TeV_sl_jge6_t3_low.txt',
    'ttH_hbb_13TeV_sl_jge6_tge4_high.txt',
    'ttH_hbb_13TeV_sl_jge6_tge4_low.txt',
]

g_pre = os.path.join(args.i, 'cadi/HIG-16-040/')
g_cards = [
    'hgg_card_13TeV_tth_moriond17.txt'
]

for card in t_cards:
    cb.ParseDatacard(t_pre + card, analysis='ttH', channel='tautau', mass='*')
...
cards = ' '.join(
    [x.replace('.txt', '=') + t_pre + x for x in t_cards] +
    [x.replace('.txt', '=') + b_pre + x for x in b_cards] +
    [x.replace('.txt', '=') + l_pre + x for x in l_cards] +
    [x.replace('.txt', '=') + g_pre + x for x in g_cards])
...
os.system('combineCards.py %s &> ttH_tautau_def.txt' % cards_t)
