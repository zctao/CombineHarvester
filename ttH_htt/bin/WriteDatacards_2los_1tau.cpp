...
int main(int argc, char** argv) {
  ...
  config.add_options()
    ("input_file,i", po::value<string>(&input_file)->default_value("datacards_2los_1tau.root"))
    ("output_file,o", po::value<string>(&output_file)->default_value("ttH_2los_1tau.root"))
    ("lumi,l", po::value<double>(&lumi)->default_value(lumi))
    ("add_shape_sys,s", po::value<bool>(&add_shape_sys)->default_value(true))
    ("bin_name,b", po::value<string>(&bin_name)->default_value("ttH_2los_1tau"));
  ...
  vector<string> bkg_procs = {"TTW", "TTZ", "EWK", "Rares", "fakes_data"};
  ...
  if ( add_shape_sys ) {
    /*
    // ----- Doesn't exist in the root file --------
    cb.cp().process(sig_procs)
